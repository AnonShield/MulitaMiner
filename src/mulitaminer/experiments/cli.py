...
Invoked as ``python tools/run_experiments.py`` (thin wrapper) or via the
installed ``mulita-experiments`` entry point; run from the repo root.
"""
import argparse
import json
import os
...
def main():
    ...
    if not args.skip_metrics and evaluation_methods:
        ...
        metrics_cmd = [
            sys.executable, "-m", "metrics.run_metrics",
