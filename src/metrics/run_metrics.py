...
Usage examples (``mulita-metrics`` — installed entry point — is equivalent to
``python tools/run_metrics.py``; run from the repo root either way)::
...
from mulitaminer.configs.constants import RUNS_DIR
...
# CWD-relative, same convention as constants.py (run from the repo root).
DEFAULT_BASELINES_DIR: Path = Path("resources") / "baselines"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Directory holding <scanner>/<target>.xlsx baselines. "
             f"Default: {DEFAULT_BASELINES_DIR.as_posix()}/.",
    )
...
def _rel(run_dir: Path) -> str:
    cwd = Path.cwd()
    return str(run_dir.relative_to(cwd) if run_dir.is_relative_to(cwd) else run_dir)
